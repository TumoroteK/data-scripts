...
--                   AND p.patient_id = id), 500) as organe,	
	...
	-- new
	cession.numero as numero_cession,
	-- new
	cession.depart_date as date_cession,
	cess_type.type as type_cession,
	cess_statut.statut as statut_cession,
	cession.etude_titre as titre_etude,
	destr_motif.motif as motif_destruction,
	COALESCE((SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select ca.champ_annotation_id from CHAMP_ANNOTATION ca inner join TABLE_ANNOTATION ta on ta.table_annotation_id = ca.table_annotation_id where ca.nom = 'N° Waybill Carboglace' and ta.nom = 'N° air Waybill Carboglace') 
	and a.objet_id=cession.cession_id),'') as 'N° Waybill Carboglace',
	COALESCE((SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select ca.champ_annotation_id from CHAMP_ANNOTATION ca inner join TABLE_ANNOTATION ta on ta.table_annotation_id = ca.table_annotation_id where ca.nom = 'N° air Waybill Combo' and ta.nom = 'N° air Waybill Combo') 
	 and a.objet_id=cession.cession_id),'') as 'N° air Waybill Combo',
	COALESCE((SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select ca.champ_annotation_id from CHAMP_ANNOTATION ca inner join TABLE_ANNOTATION ta on ta.table_annotation_id = ca.table_annotation_id where ca.nom = 'Waybill number T°C AMB' and ta.nom = 'N° air Waybill T°C AMB') 
	 and a.objet_id=cession.cession_id),'') as 'Waybill number T°C AMB',
	COALESCE((SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select ca.champ_annotation_id from CHAMP_ANNOTATION ca inner join TABLE_ANNOTATION ta on ta.table_annotation_id = ca.table_annotation_id where ca.nom = 'N° air Waybill Refrig' and ta.nom = 'N° air Waybill Refrig') 
	 and a.objet_id=cession.cession_id),'') as 'N° air Waybill Refrig'
FROM 
-- new
CESSION cession
JOIN CEDER_OBJET cdobj on cdobj.cession_id = cession.cession_id
JOIN ECHANTILLON e on cdobj.objet_id = e.echantillon_id and entite_id = 3 
-- fin du new
...
LEFT JOIN CESSION_TYPE cess_type ON cession.cession_type_id = cess_type.cession_type_id 
LEFT JOIN CESSION_STATUT cess_statut ON cession.cession_statut_id = cess_statut.cession_statut_id 
LEFT JOIN DESTRUCTION_MOTIF destr_motif ON cession.destruction_motif_id = destr_motif.destruction_motif_id and destr_motif.plateforme_id = b.plateforme_id
WHERE 
	cession.depart_date between '2024-01-01-00:00:00' and '2024-12-31-23:59:59'
	and
