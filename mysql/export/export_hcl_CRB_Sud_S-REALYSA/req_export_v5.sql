...
select 'Code échantillon','Type d''échantillon','Quantité','Quantité initiale','Unité de la quantité','Date de stockage','Délai de congélation','Opérateur','Emplacement','Statut', 'cession CRBS-2022-96','Mode de préparation','Stérile','Conforme après traitement','Non conformité de l''échantillon après traitement','Conforme pour la cession','Non conformité de l''échantillon à la cession','Nb. dérivés','Evts de stockage','Code prélèvement','N° Laboratoire','Nature','Date de prélèvement','Type de prélèvement','Stérile','Risque Infectieux','Conforme à l''arrivée','Non conformité à l''arrivée','Etablissement préleveur','Service préleveur','Type de conditionnement','Nombre de conditionnements','Milieu','Statut juridique','Date du statut juridique','Date de départ','Transporteur','Temp. de transport','Date d''arrivée','Opérateur','Quantité','N° de dossier patient','Nb total d''échantillons','Nb d''échantillons restants','Nb échantillons stockés','Age au prélèvement','Nb produits dérivés','Nre PAXgene et vol','Nbre STRECK et vol','Nbre tubes secs et vol','Libellé','Nom','Sexe','Nb prélèvements'
union all
SELECT e.code as code_echantillon, 
	...
	(SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select champ_annotation_id from CHAMP_ANNOTATION where nom = 'Nre PAXgene et vol') and a.objet_id=p.prelevement_id) as Nre_PAXgene_et_vol,
	(SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select champ_annotation_id from CHAMP_ANNOTATION where nom = 'Nbre STRECK et vol') and a.objet_id=p.prelevement_id) as Nbre_STRECK_et_vol,
	(SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select champ_annotation_id from CHAMP_ANNOTATION where nom = 'Nbre tubes secs et vol') and a.objet_id=p.prelevement_id) as Nbre_tubes_secs_et_vol,
	m.libelle,
	pat.nom,
	pat.sexe,
...
-- v5 : permet de savoir si les échantillons ont été cédés dans la cession CRBS-2022-96 Externalisation ADN/Serum REALYSA
LEFT JOIN CEDER_OBJET cedobj ON cedobj.objet_id = e.echantillon_id and cedobj.entite_id = 3 and cedobj.cession_id = 6623
 WHERE 
	b.NOM = 'S-REALYSA'
-- and p.date_prelevement < '2025-04-01 00:00:00'
;
