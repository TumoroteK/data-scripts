...
SELECT 
	...
	-- new spécifique
	(SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select champ_annotation_id from CHAMP_ANNOTATION where nom = '% VIABILITE BT') and a.objet_id=e.echantillon_id) as VIABILITE_BT,
	-- new 
	e.prelevement_id, 
...
--            WHERE prel.prelevement_id = id) as 'Diagnostic',
	...
	-- --------------------------- /!\ annotation
	-- new spécifique
	(SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select champ_annotation_id from CHAMP_ANNOTATION where nom = 'Remarque') and a.objet_id=p.prelevement_id) as Remarque_prelevement,
	m.libelle,
	-- pat.nom,
	pat.sexe,
    -- new : ORGANE /!\ 
...
--                   AND p.patient_id = id), 500) as organe,	
	...
	(SELECT a.alphanum from ANNOTATION_VALEUR a where a.champ_annotation_id=(select champ_annotation_id from CHAMP_ANNOTATION where nom = 'Remarques' and table_annotation_id = 18) and a.objet_id=pat.patient_id) as Remarques_patient
FROM 
-- new
CESSION cession
...
WHERE 
	cession.depart_date between '2023-01-01-00:00:00' and '2023-12-31-23:59:59'
	and
