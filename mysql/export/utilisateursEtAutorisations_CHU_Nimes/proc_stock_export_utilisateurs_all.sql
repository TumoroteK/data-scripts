...
DROP PROCEDURE IF EXISTS z_specific_export_utilisateurs_all &&
CREATE PROCEDURE z_specific_export_utilisateurs_all ()
BEGIN
    DECLARE query TEXT DEFAULT '';

    SET @query = 
    "select autres_data.login, autres_data.collaborateur, autres_data.service, autorisations.autorisation, autres_data.plateforme_origine, autres_data.date_creation, autres_data.archive, autres_data.date_desactivation
    from 
    (
        select u.login, CONCAT(c.nom, ' ', c.prenom) as collaborateur, GROUP_CONCAT(s.nom SEPARATOR ', ') as service, CONCAT(p.NOM) as plateforme_origine, date_format(o.date_, '%d/%m/%Y %H:%i:%s') as date_creation, u.archive, date_format(u.timeout, '%d/%m/%Y') as date_desactivation
        from UTILISATEUR u
        ...
        group by u.login, c.nom, c.prenom, p.NOM, o.date_, u.archive, u.timeout
    ) as autres_data
    inner join (
