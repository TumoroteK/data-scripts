...
mysql -u$user -p$pass -e "show tables from $schema"| awk '{if(NR>1) print "show index from "$1";"}'| mysql -u$user -p$pass $schema| awk '{if($3 !~ /Key_name/ && $3 !~ /PRIMARY/) print $1" "$3}'| awk '{print "alter table "$1" drop constraint "$2";"}'
